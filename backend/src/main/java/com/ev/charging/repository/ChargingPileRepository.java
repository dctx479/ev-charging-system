...
public interface ChargingPileRepository extends JpaRepository<ChargingPile, Long> {
     ...
     * @return 充电桩列表
     */
    List<ChargingPile> findByStationIdAndStatus(Long stationId, String status);
     ...
     * @return 故障充电桩列表
     */
    @Query("SELECT p FROM ChargingPile p WHERE p.status = 'FAULT' ORDER BY p.updateTime DESC")
