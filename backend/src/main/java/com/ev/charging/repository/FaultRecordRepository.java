...
public interface FaultRecordRepository extends JpaRepository<FaultRecord, Long> {
    ...
    /**
     * 全局统计指定修复状态的故障数量（不按站点过滤）
     */
    @Query("SELECT COUNT(f) FROM FaultRecord f WHERE f.repairStatus = :repairStatus")
    Long countGlobalByRepairStatus(@Param("repairStatus") Byte repairStatus);
