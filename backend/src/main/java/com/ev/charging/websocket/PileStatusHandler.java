 ...
 *
 * 安全改进:
 * 1. 从握手拦截器验证后的attributes获取userId
 * 2. 如果userId为null（未通过认证），则拒绝连接
 * 3. 记录所有连接的userId便于审计
 * 4. 使用@Component注解确保Spring容器管理单例
 */
@Slf4j
@Component
public class PileStatusHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // userId应该已经由JwtHandshakeInterceptor验证和设置
        // 如果为null说明握手拦截器有问题，直接拒绝连接
