...
CREATE TABLE `users` (
    ...
    `status` VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' COMMENT '状态：ACTIVE-正常，DISABLED-禁用，PENDING-待审核，REJECTED-已拒绝，SUSPENDED-已停用',
    `reject_reason` VARCHAR(500) DEFAULT NULL COMMENT '审核拒绝原因',
    `service_area` VARCHAR(200) DEFAULT NULL COMMENT '服务区域（代充师傅专用）',
    `company_name` VARCHAR(100) DEFAULT NULL COMMENT '企业名称（企业运营商专用）',
    `business_license` VARCHAR(18) DEFAULT NULL COMMENT '企业统一社会信用代码（企业运营商专用）',
    `id_card` VARCHAR(18) DEFAULT NULL COMMENT '身份证号（代充师傅专用）',
...
CREATE TABLE `charge_order` (
    ...
    `charge_mode` TINYINT DEFAULT NULL COMMENT '充电模式：1按时间 2按电量 3充满',
    `target_value` DECIMAL(10,2) DEFAULT NULL COMMENT '目标值（时间或电量）',
    `payment_method` TINYINT DEFAULT NULL COMMENT '支付方式：1微信 2支付宝 3余额',
...
CREATE TABLE `carbon_credit_record` (
    ...
    `carbon_reduction` DECIMAL(6,2) DEFAULT NULL COMMENT '对应碳减排量(kg)',
...
-- 3.2.1 积分待发放记录表（补偿机制）
DROP TABLE IF EXISTS `credit_pending_record`;
CREATE TABLE `credit_pending_record` (
    `id` BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '记录ID',
    `order_id` BIGINT NOT NULL COMMENT '订单ID',
    `user_id` BIGINT NOT NULL COMMENT '用户ID',
    `charge_amount` DECIMAL(10,2) NOT NULL COMMENT '充电量(kWh)',
    `retry_count` INT NOT NULL DEFAULT 0 COMMENT '重试次数',
    `last_error` VARCHAR(500) DEFAULT NULL COMMENT '最后一次错误信息',
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '状态：0-待重试 1-已成功 2-已放弃',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    FOREIGN KEY (`order_id`) REFERENCES `charge_order`(`id`),
    FOREIGN KEY (`user_id`) REFERENCES `user`(`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分待发放记录表';
...
CREATE TABLE `valet_charger` (
    ...
    `user_id` BIGINT DEFAULT NULL COMMENT '关联用户ID（用于认证和所有权验证）',
    ...
    `service_price` DECIMAL(6,2) DEFAULT 30.00 COMMENT '服务价格',
...
CREATE TABLE `operation_log` (
    ...
    `operation_type` VARCHAR(20) NOT NULL COMMENT '操作类型：CREATE/UPDATE/DELETE/QUERY/REVIEW',
...
CREATE TABLE `credit_product` (
    ...
    `category` VARCHAR(20) NOT NULL COMMENT '商品类型：coupon/discount/gift',
...
-- BCrypt 哈希值: $2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f. (对应密码: SHA256(123456)，前端SHA256后BCrypt)
INSERT INTO `users` (`username`, `password`, `nickname`, `phone`, `email`, `user_type`, `balance`, `carbon_credits`, `status`) VALUES
-- 管理员账号
('admin', '$2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f.', '系统管理员', '13800138000', 'admin@evcharge.com', 'ADMIN', 10000.00, 0, 'ACTIVE'),
-- 普通用户账号
('zhangsan', '$2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f.', '张三', '13800138001', 'zhangsan@test.com', 'USER', 500.00, 1200, 'ACTIVE'),
('lisi', '$2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f.', '李四', '13800138002', 'lisi@test.com', 'USER', 300.00, 850, 'ACTIVE'),
('wangwu', '$2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f.', '王五', '13800138003', 'wangwu@test.com', 'USER', 200.00, 2100, 'ACTIVE')
ON DUPLICATE KEY UPDATE update_time = NOW();

-- 5.3 插入业务用户（user表，用于业务数据关联）
-- BCrypt 哈希值: $2b$10$8p3yCwfSc54OF5aARdntcufMrLxcbvkToO50LDPt4EX7/7Izby2f.
...
VALUES
-- 北京地区
(1, '北京朝阳大悦城充电站', '北京市', '北京市', '朝阳区', '朝阳北路101号大悦城停车场', 116.5000, 39.9200, 20, 15, 5.00, '00:00-24:00', 1, 1, 100.00, 200.00, 1),
(2, '北京国贸充电站', '北京市', '北京市', '朝阳区', '建国门外大街1号国贸中心', 116.4590, 39.9091, 15, 10, 8.00, '06:00-23:00', 0, 0, 0.00, 0.00, 1),
(3, '北京西单充电站', '北京市', '北京市', '西城区', '西单北大街131号', 116.3743, 39.9074, 12, 8, 6.00, '00:00-24:00', 0, 0, 0.00, 0.00, 1),

-- 上海地区
(2, '上海浦东国际机场充电站', '上海市', '上海市', '浦东新区', '机场大道900号', 121.8054, 31.1443, 30, 25, 8.00, '00:00-24:00', 1, 0, 150.00, 0.00, 1),
...
-- 深圳地区
(3, '深圳南山科技园充电站', '广东省', '深圳市', '南山区', '科技园南路', 113.9500, 22.5400, 25, 20, 6.00, '00:00-24:00', 1, 1, 120.00, 150.00, 1),
(1, '深圳宝安机场充电站', '广东省', '深圳市', '宝安区', '领航四路', 113.8167, 22.6000, 28, 22, 7.00, '00:00-24:00', 1, 0, 80.00, 0.00, 1),
(2, '深圳福田中心区充电站', '广东省', '深圳市', '福田区', '深南大道', 114.0580, 22.5455, 16, 11, 8.00, '00:00-24:00', 0, 0, 0.00, 0.00, 1),

-- 广州地区
(2, '广州天河城充电站', '广东省', '广州市', '天河区', '天河路208号', 113.3171, 23.1348, 14, 9, 6.00, '08:00-23:00', 0, 0, 0.00, 0.00, 1),

-- 杭州地区
(3, '杭州西湖文化广场充电站', '浙江省', '杭州市', '下城区', '中山北路口', 120.1650, 30.2800, 12, 8, 5.00, '00:00-24:00', 1, 0, 60.00, 0.00, 1),

-- 成都地区
(1, '成都春熙路充电站', '四川省', '成都市', '锦江区', '春熙路', 104.0800, 30.6600, 15, 10, 5.00, '07:00-23:00', 0, 0, 0.00, 0.00, 1)
ON DUPLICATE KEY UPDATE update_time = NOW();
