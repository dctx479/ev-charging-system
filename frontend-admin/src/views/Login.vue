...
const handleLogin = async () => {
  ...
  await loginFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      adminStore.login(res.data.token, res.data.userInfo, expiresIn)
      resetLoginRedirectTime()

      ElMessage.success('登录成功')

      // 跳转到 Dashboard 或重定向页面（防止开放重定向）
      const redirect = route.query.redirect || '/dashboard'
      const safeRedirect = (typeof redirect === 'string' && redirect.startsWith('/') && !redirect.startsWith('//')) ? redirect : '/dashboard'
