  ...
  <div class="fault-management">
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-pagination
        ...
        @size-change="() => { queryParams.page = 0; fetchFaultList() }"
        @current-change="(val) => { queryParams.page = val - 1; fetchFaultList() }"
...
import { getAllPiles, updatePileStatus } from '@/api/pile'
...
const fetchFaultList = async () => {
  ...
  try {
    ...
    // 处理分页响应：Page<T> 对象返回的字段为 content 和 totalElements
...
const handleComplete = async (fault) => {
  // 检查当前状态是否允许完成维修
  if (fault.repairStatus !== 1) {
    ElMessage.warning(`只有"维修中"状态的故障可以完成维修，当前状态为"${getRepairStatusText(fault.repairStatus)}"`)
    return
  }
  ...
  ElMessageBox.confirm('确认该故障已维修完成？', '提示', {
    ...
    try {
      ...
      ElMessage.success('维修完成，充电桩已恢复正常')
      fetchFaultList()
    } catch (error) {
