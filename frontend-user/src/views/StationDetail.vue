<template>
  <div class="station-detail">
    ...
    <div v-else-if="station" class="content">
      ...
      <!-- 充电搭子入口 -->
      <van-cell-group inset title="充电搭子">
        <van-cell
          title="找个充电搭子聊聊天"
          icon="chat-o"
          is-link
          @click="goToChargingBuddy"
        >
          <template #label>
            等待充电时，和同站车主聊天互动
          </template>
      ...
      <!-- 周边服务入口 -->
      <van-cell-group inset title="周边服务">
        <van-cell
          title="搜索周边POI"
          icon="search"
          is-link
          @click="goToNearbySearch"
        >
          <template #label>
            查找附近的餐饮、娱乐、购物等服务
          </template>
...
const goToChargingBuddy = () => {
  router.push(`/chat/buddy?stationId=${station.value.id}`)
}

const goToNearbySearch = () => {
  // 跳转到周边POI搜索页面，传递充电站坐标
  router.push({
    path: '/nearby/search',
    query: {
    ...
    markerInstance.value.on('click', () => {
      const infoWindow = new window.AMap.InfoWindow({
        content: `<div style="padding: 8px;">
          <h4 style="margin: 0 0 8px 0; font-size: 14px;">${escapeHtml(station.value.name)}</h4>
          <p style="margin: 0; font-size: 12px; color: #666;">${escapeHtml(station.value.address)}</p>
        </div>`
      })
...
const onMapSelect = (action) => {
  ...
  switch (mapType) {
    ...
    case 'baidu':
      ...
      // 如果有用户位置，添加起始地参数
      if (hasUserPosition) {
        navUrl = `https://api.map.baidu.com/direction?origin=latlng:${fromLat},${fromLng}|name:${encodeURIComponent('我的位置')}&destination=latlng:${latitude},${longitude}|name:${encodeURIComponent(name)}&mode=${baiduMode}&coord_type=wgs84&output=html`
      } else {
        navUrl = `https://api.map.baidu.com/direction?destination=latlng:${latitude},${longitude}|name:${encodeURIComponent(name)}&mode=${baiduMode}&coord_type=wgs84&output=html`
      }
      break
