<template>
  <div class="queue-status">
    ...
    <div class="content" v-if="queueInfo">
      ...
      <!-- 已叫号提示 -->
      <div v-if="queueInfo.status === 2" class="call-notice">
        <van-notice-bar
          :text="noticeText"
          background="#fff7e6"
          color="#ff9800"
      ...
      <div class="button-group">
        ...
        <van-button
          v-if="queueInfo.status === 1 || queueInfo.status === 2"
          type="danger"
          size="large"
          plain
          :loading="leaving"
        ...
        <van-button
          v-if="queueInfo.status === 3 || queueInfo.status === 4"
          type="primary"
          size="large"
          @click="goToStation"
        >
...
const noticeText = computed(() => {
  ...
  if (!queueInfo.value.expireTime) return '已轮到您充电！请尽快到达充电站'

  const expireTime = dayjs(queueInfo.value.expireTime)
  const now = dayjs()
  const remaining = Math.max(0, expireTime.diff(now, 'minute'))
...
const loadQueueStatus = async () => {
  try {
    ...
    queueInfo.value = res.data  // null when no queue record (backend returns success with null data)
    loading.value = false
  } catch (error) {
    showToast('加载失败')
    loading.value = false
...
const handleStartCharging = () => {
  if (!queueInfo.value.pileId) {
    showToast('充电桩尚未分配，请稍候')
    return
  }
  // 跳转到创建订单页面
  router.push({
