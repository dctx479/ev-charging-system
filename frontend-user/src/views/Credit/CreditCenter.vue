        ...
        <div v-if="records.length > 0" class="records-list">
          ...
          >
            <div class="record-left">
              ...
              <div class="record-time">{{ formatTimeRelative(record.createTime) }}</div>
            </div>
...
import { formatTimeRelative } from '@/utils/dateFormat'
...
const loadStatistics = async () => {
  try {
    ...
    showToast(error.response?.data?.message || '加载数据失败')
  }
}
...
const handleCheckIn = async () => {
  try {
    ...
    showFailToast(error.response?.data?.message || error.message || '签到失败')
  }
}
